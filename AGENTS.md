...
- Ticket #5 SPA: custom React (not Gradio — must host react-konva); one shared React canvas for dimension lines and lasso; viewer is three.js (not model-viewer — needs Raycaster for #6); 3MF is a download artifact, never rendered inline.
