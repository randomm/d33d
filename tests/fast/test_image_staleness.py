...
``docs/bosl2-pinning.md`` must carry the ``--label`` argument and the
``-t`` tag must match ``d33d.render_worker.RENDER_WORKER_IMAGE`` — the
doc previously showed ``-t d33d-render-worker:latest``, which did not match
the constant and drifted from the code.
"""
...
def test_doc_build_command_has_label_arg() -> None:
    src = DOCS_BUILD_CMD.read_text(encoding="utf-8")
    block = _extract_build_command_block(src)
    assert "--label" in block, (
        "canonical build command in docs/bosl2-pinning.md is missing the "
        "--label argument (issue #236)"
    )
